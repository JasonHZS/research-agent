...
### Running the Agent
```bash
# Interactive mode (default provider: Aliyun deepseek-v4-flash)
python -m src.main
...
1. **Aliyun (Default)**: Uses DashScope API with OpenAI-compatible endpoint
   - Models: `deepseek-v4-flash` (default), `qwen3.6-plus`, `kimi-k2.6`, `glm-5.1`, `deepseek-v4-pro`
...
3. **OpenRouter**: OpenAI-compatible gateway
   - Default model: `openai/gpt-5`
   - Requires `OPENROUTER_API_KEY`

Model configuration is handled by `_get_model_config()` and `create_llm()` which create ChatOpenAI instances for custom endpoints.
