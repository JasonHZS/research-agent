...
4. **综合报告与栏目化编排**：
   ...
   **绝不（MUST NEVER）**对`content-reader-agent`输出的内容进行修改、重写或再次总结，仅允许结构性归类与重排。
   在处理`content-reader-agent`返回的内容时，保留其中`![alt](url)`格式的 image_url 引用。
